...
import { validateRequest, institutionUpdateSchema } from '@/lib/validation';
...
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    ...
    logger.error('Failed to get institution', error as Error);
...
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    const validation = validateRequest(institutionUpdateSchema, body);
    ...
    const repository = new InstitutionRepository(provider);
    const service = new InstitutionService(repository);
    const institution = await service.update(id, validation.data);
    return NextResponse.json({ data: institution });
  } catch (error) {
    logger.error('Failed to update institution', error as Error);
    if (error instanceof NotFoundError) {
      return NextResponse.json({ error: error.message }, { status: 404 });
    }
    if (error instanceof AppError) {
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  try {
    ...
    logger.error('Failed to delete institution', error as Error);
    if (error instanceof NotFoundError) {
      return NextResponse.json({ error: error.message }, { status: 404 });
    }
    if (error instanceof AppError) {
