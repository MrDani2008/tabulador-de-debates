...
import { validateRequest, roundUpdateSchema } from '@/lib/validation';
...
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    ...
    const round = await service.getById(id);
    return NextResponse.json({ data: round });
  } catch (error) {
    logger.error('Failed to get round', error as Error);
...
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    const validation = validateRequest(roundUpdateSchema, body);
    ...
    const repository = new RoundRepository(provider);
    const service = new RoundService(repository);
    const round = await service.update(id, validation.data);
    return NextResponse.json({ data: round });
  } catch (error) {
    logger.error('Failed to update round', error as Error);
    if (error instanceof NotFoundError) {
      return NextResponse.json({ error: error.message }, { status: 404 });
    }
    if (error instanceof AppError) {
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  try {
    ...
    logger.error('Failed to delete round', error as Error);
    if (error instanceof NotFoundError) {
      return NextResponse.json({ error: error.message }, { status: 404 });
    }
    if (error instanceof AppError) {
